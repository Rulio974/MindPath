...
from .schemas import (
    ...
    SuccessResponse, ErrorResponse, TokenAuth
)
...
@auth_router.post("/verify-token", response_model=UserSchema)
...
async def get_current_user_info(
    ...
    user = UserCRUD.get_user(db, current_user["user_id"])
    if not user:
        raise HTTPException(
...
async def update_current_user(
    ...
    updated_user = UserCRUD.update_user(db, current_user["user_id"], user_update)
    if not updated_user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Utilisateur non trouvé"
...
async def regenerate_token(
    ...
    """Régénère le token API de l'utilisateur actuel"""
    new_token = UserCRUD.regenerate_token(db, current_user["user_id"])
    if not new_token:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Utilisateur non trouvé"
